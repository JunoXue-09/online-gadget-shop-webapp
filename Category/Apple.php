		...
		<main class="Container">
			<div class="apple-banner">
				<div class="banner-content">
					<h2>UNLOCK THE POWER:<br>Pre-Order iPhone 17 Lineup</h2>
			...
			<?php
				...
				if (!$conn) {
					...
					// Filter products specifically for the Apple category
					$sql = "SELECT * FROM product WHERE category = 'Apple'";
					...
					if (!$result || mysqli_num_rows($result) === 0) {
						echo '<p>No Apple products found.</p>';
					} else {
						while ($row = mysqli_fetch_assoc($result)) {
							$id = htmlspecialchars($row['id']); // Updated to match your schema's primary key
